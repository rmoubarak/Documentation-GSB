...
namespace GSB
{
    public partial class FrmVisiteMaj : FrmBase
    {
        ...
        {
            // Récupérer l'id et la date visite
            int id = laVisite.Id;
            DateTime uneDate = dtpDate.Value;

            // Confirmation avant de modifier
            DialogResult result = MessageBox.Show("Voulez-vous vraiment modifier cette visite ?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
            {
                // Récupérer la visite
                Visite uneVisite = (Visite)dgvVisitesMaj.SelectedRows[0].Cells[0].Value;
                string message;
                // Appelle de la méthode modifierRendezVous de la classe Passerelle
                ...
                if (ok)
                {
                    ...
                    MessageBox.Show("La visite a été modifiée avec succès.", "Confirmation", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
                    MessageBox.Show("Erreur lors de la modification : " + message, "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        {
            {
                ...
                DialogResult result = MessageBox.Show("Voulez-vous vraiment supprimer cette visite ?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (result == DialogResult.Yes)
                {
                    // Récupérer la visite
                    int id = laVisite.Id;
                    ...
                    // Appelle de la méthode supprimerRendezVous de la classe Passerelle
                   bool ok = Passerelle.supprimerRendezVous(id, out message);
                    if (ok)
                    {
                        // Supprimer la visite
                        ...
                        remplirdgvVisitesMaj();
                        // Afficher un message de confirmation
                        MessageBox.Show("La visite a été supprimée avec succès.", "Confirmation", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("Erreur lors de la suppression : " + message, "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
        ...
        private void remplirdgvVisitesMaj()
        {
            ...
            {
                MessageBox.Show("Aucune visite programmée.", "Erreur", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                panelDroit.Visible = false;
                panelGauche.Visible = false;
            }
        }
